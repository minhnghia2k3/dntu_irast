        ...
        <>
            <Link
                ...
                <div className="w-[70px] h-[70px] md:w-[100px] md:h-[100px] mr-2 relative shadow-md rounded-full">
                    <Image src={data.logo} alt="Logo" fill className="rounded-full" />
                </div>
                {
                    showTag ? (
                        <div className="flex justify-center items-start flex-col gap-1 drop-shadow">
                            <h1 className="text-base md:text-lg font-semibold text-red-primary line-clamp-1 truncate whitespace-nowrap inline-block md:max-w-[400px] max-w-[200px] w-full text-start">{data.name}</h1>
                        ...
                        <div className="flex justify-center items-start flex-col gap-1 drop-shadow w-fit">
                            <h1 className={`text-base md:text-lg font-semibold text-red-primary line-clamp-1 truncate whitespace-nowrap inline-block md:max-w-[400px] max-w-[200px] w-full text-start`}>{data.name}</h1>
                            <p className={`text-xs md:text-sm text-gray-400 line-clamp-1 truncate whitespace-nowrap w-full text-start md:max-w-none max-w-[200px]`}>{data.contact}</p>
